...
const imgStickManParty = require('../imgs/happy.png');
...
export default class CenaDetalhesEventoLista extends Component {
  constructor(props){
    super(props);
    //this.state = { evento : this.getEventos().filter((evento) => evento.evID == this.props.evID)};
    this.state = { evento : []};
	this.state = {visible: false};
	this.state = {codPromoter: ''};
	this.state = {statusVisualizaCod: false};
    this.state = { codListaUsuario : []};
  ...
  componentWillMount() {
    this.listarDados();
    this.verificaStatusUsuarioNaBalada();
  }

verificaStatusUsuarioNaBalada(){
	...
	var usuarios = firebaseRef.child('user').child(usuarioAtual.uid);
		...
		if(snapshot.child('/codListaVip' + '/evID/' + this.props.evID).exists()){
			...
			if(snapshot.child('/codListaVip' + '/evID/' + this.props.evID).val().codUsado){
			   ...
			   //obtem o codigo do usuario para aquela balada
			   const usuarioAtual = auth.currentUser;
			   var usuarios = firebaseRef.child('user').child(usuarioAtual.uid).child('codListaVip').child('evID').child(this.props.evID);
			   usuarios.on('value', (snapshot) => { 
			      var usuario = snapshot.val();
			      	this.setState({ codListaUsuario : usuario.cod});
			      	this.setState({ statusVisualizaCod : usuario.codVisualizado});
		...
		//verifica se o usuario é um promoter dessa balada.
		if(snapshot.child('/Promoter' + '/evID/' + this.props.evID).exists()){
			//Caso exista quer dizer que o usuario logado é promoter da balada.
			this.setState({statusCadastroLista : 'promoter'});
			var promoter = snapshot.child('/Promoter' + '/evID/' + this.props.evID).val()
			this.setState({codPromoter : promoter.cod});
		}
		});	
}
...
returnListaVIP(){
	  	if(this.state.statusCadastroLista == ''){
	  		...
	  		return(
				  <View style={styles.listaVIP}>
			          <View style={{flex: 1}}>
			              <View style={{flex: 4, alignItems: 'center', justifyContent: 'center', flexDirection:'row', paddingTop: 30}}>
			                 <View style={{alignItems: 'flex-start', justifyContent: 'center', paddingLeft: 10}}>
			                   <Text style={{color: 'white', fontSize: 20, fontWeight: 'bold',marginBottom: 10}}>Mãe tô na balada!</Text>
			                   <Text style={{color: 'white', fontSize: 16}}>Agora que você já aproveitou,</Text>
			                   <Text style={{color: 'white', fontSize: 16}}>garanta o VIP dos seus amigos</Text>
			                   <Text style={{color: 'white', fontSize: 16}}>pras próximas baladas!</Text>
			                 </View>    
			                 <View style={{alignItems: 'flex-start', justifyContent: 'center'}}>
								<Image style={{backgroundColor: 'transparent', width: 85, height: 120}} source={imgStickManParty}></Image>
			                 </View>  			                  
			              </View>
			              <View style={{flex: 4, alignItems: 'center', justifyContent: 'center', marginBottom: 15}}>
		                      <TouchableHighlight style={styles.btnConvidarAmigos}
		                        onPress={() => {
		                        this.onOpen()
		                      }}>
		                        <Text style={styles.txtConvidarAmigos}>CONVIDAR AMIGOS</Text>
		                      </TouchableHighlight>     
			              </View>
			              <BotaoInserirNomeListaVIP evID={this.state.evento.evID} abreModal={false} codListaVisualizado={this.state.statusVisualizaCod} codlista={this.state.codListaUsuario}/>
			          </View>  	
				  </View>
	  			);
			          ...
			          <View style={{flex: 1}}>
			              <View style={{flex: 4, alignItems: 'center', justifyContent: 'center'}}>
			                 <View style={{alignItems: 'center', justifyContent: 'center'}}>
			                   <Text style={{color: 'white', fontSize: 24, fontWeight: 'bold',marginBottom: 10}}>Aeee! VIP Garantido!</Text>
			                   <Text style={{color: 'white', fontSize: 16}}>É só chegar chegando no dia da balada com</Text>
			                   <Text style={{color: 'white', fontSize: 16}}>um documento e falar o código secreto:</Text>
			                   <Text style={{color: 'white', fontSize: 24, fontWeight: 'bold', marginTop: 10}}>{this.state.codListaUsuario}</Text>
			                 </View>     
			              </View>
				  ...
				  <View style={styles.listaVIP}>
			          <View style={{flex: 1}}>
			             ...
			             <View style={{flex: 2, margin: 10}}>
			              ...
			              <View style={{flex: 4, alignItems: 'center', justifyContent: 'center', marginBottom: 15}}>
			              	<BotaoInserirNomeListaVIP evID={this.state.evento.evID} abreModal={true} codListaVisualizado={true}/>           
			              </View>
			          </View>  	
				  </View>
	  			);
	  	} else if(this.state.statusCadastroLista == 'promoter'){
	  		return(
				  <View style={styles.listaVIP}>
			          <View style={{flex: 1}}>
			              <View style={{flex: 2, alignItems: 'center', justifyContent: 'center'}}>
			                 <View>
			                   <Text style={{color: 'white', fontSize: 26}}>VIPS DISPONÍVEIS</Text>
			                 </View>     
			              </View>
			             <View style={{flex: 2, margin: 10}}>
			                <Text style={{color: 'white', fontSize: 14, textAlign:'center'}}>Verifique aqui o numero de pessoas que já se </Text>
			                <Text style={{color: 'white', fontSize: 14, textAlign:'center'}}>inscreveu como VIP na sua festa</Text>
			              </View>
			              <View style={{flex: 4, alignItems: 'center', justifyContent: 'center', marginBottom: 15}}>
			              	<BotaoLiberarVips evID={this.state.evento.evID} abreModal={true} codListaVisualizado={true}/>           
			              </View>
			          </View>  	
				  </View>
	  			);	  		
  ...
  returnEventsListView(){
	if (this.state.evento != undefined){
		return(
				<View style={{height:1350, flex: 1, backgroundColor: '#1D1D1D'}}>
